...
## Project Design Doc (Version 1, Not Final)
...
graph TD  
    ...
    subgraph AI \["AI Integration"\]  
        18\["Frontier Model API (Provider TBD)"\]  
    end
...
Path: `/api/v1/campus/chat`

Purpose: Primary entry point for student queries about campus life

Request Format: JSON 	`message, conversation_id, context` (optional)

`{`

  `"message": "When is the next career fair?",`
...
Response Format: JSON 	`response`, `sources`
...
`{`
  ...
  `"data": {`
    ...
    `"response": "The next career fair is scheduled for March 15th at the Student Union.",`
    ...
    `"sources": [`
      ...
      `{`
        ...
        `"title": "Spring 2026 Campus Events Calendar",`
...
Path: `/api/v1/campus/status`

Purpose: To retrieve real-time status updates and scheduled events across the campus.

Request Format: Query parameters

Response Format: JSON	`timestamp`, `data`
...
`{`
  ...
  `"data": [`
    ...
    `{ "id": 1, "event": "Hackathon", "location": "Lab A", "status": "Ongoing" },`

    `{ "id": 2, "event": "Library Closure", "location": "Main Library", "status": "Maintenance" }`
...
Path: `/api/v1/campus/ingest`

Purpose: To manually trigger scraping and vectorization of new campus documents

Request Format: JSON 	`sourceURL`, `type`

`{`

  `"sourceUrl": "https://campus.edu/news/latest",`

  `"type": "news_article"`

`}`

Response Format: JSON 	`status`, `jobID, embeddingsGenerated`
...
- Dining Hrs/Menus (Hrs vary around breaks and also weather)  
- Campus Events
...
The system will format all AI outputs to conform strictly to the standardized MaristChat API JSON envelope for the `/api/campus/chat` endpoint.
...
| Weeks 10 \- 11 | **Milestone 2: RAG Pipeline & API Integration  Deliverable:** A completed `POST /api/campus/chat` endpoint that uses a centralized LLM wrapper to query frontier model and returns the standardized MAPLE JSON response envelope. |
| Week 12 | **Milestone 3: Automated QA & Evaluation  Deliverable:** An automated Node.js testing script that runs a "Golden Dataset" of hardcoded queries to verify retrieval accuracy (testing the 0.70 similarity threshold) and answer faithfulness before user testing begins. |
...
AI has been used to aid the creation of this document.
